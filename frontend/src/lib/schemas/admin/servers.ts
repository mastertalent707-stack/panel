...
export const adminServerUpdateSchema = z.object({
  ownerUuid: z.uuid().optional(),
  eggUuid: z.uuid().optional(),
  backupConfigurationUuid: z.uuid().nullable().optional(),
  ...
  limits: z
    ...
    .optional(),
  pinnedCpus: z.array(z.number()).optional(),
  startup: z.string().min(1).max(8192).optional(),
  image: z.string().min(2).max(255).optional(),
  timezone: z.string().min(3).max(255).optional(),
  featureLimits: z
    .object({
      allocations: z.number().min(0),
      databases: z.number().min(0),

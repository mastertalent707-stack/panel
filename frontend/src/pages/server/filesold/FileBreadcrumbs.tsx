...
export default function FileBreadcrumbs({
  ...
  const items = useMemo(() => {
    const items: ReactNode[] = [
      ...
      <NavLink
        ...
        to={
          browsingBackup
            ? `/server/${server?.uuidShort}/files-old?${createSearchParams({
                directory: `/.backups/${browsingBackup.uuid}`,
              })}`
            : `/server/${server?.uuidShort}/files-old`
        }
      ...
      ...pathItems.slice(browsingBackup ? 2 : 0).map((item, index) =>
        index === pathItems.length - 1 && inFileEditor ? (
          ...
          <NavLink
            key={item.path}
            to={`/server/${server?.uuidShort}/files-old?${createSearchParams({ directory: item.path })}`}
            className=' text-blue-300 hover:text-blue-200'
  ...
  return (
    <div className='flex flex-row items-center justify-between'>
      ...
      <NavLink to={`/server/${server?.uuidShort}/files-old`} hidden={!browsingBackup || inFileEditor}>

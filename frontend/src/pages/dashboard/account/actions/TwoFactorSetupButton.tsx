  ...
  const doEnable = () => {
    ...
    enableTwoFactor(code, password)
      .then(({ recoveryCodes }) => {
        ...
        addToast(
          "Two-factor authentication enabled. Please copy your recovery codes.",
          "warning"
        );
    ...
    <>
      <MantineModal.Stack>
        ...
        >
          <Stack>
            <Text>
              Help protect your account from unauthorized access. You&apos;ll be
              prompted for a verification code each time you sign in.
            </Text>
            {!token ? (
              <Spinner.Centered />
            ) : (
              <div className={"flex flex-col items-center justify-center my-4"}>
                <div
                  className={
                    "flex items-center justify-center w-56 h-56 p-2 bg-gray-50 shadow"
                  }
                >
            ...
            <Text>
              Scan the QR code above using the two-step authentication app of
              your choice. Then, enter the 6-digit code generated into the field
              below.
            </Text>
            ...
            <CopyOnClick content={recoveryCodes.join("\n")}>
              <Code block className={"grid grid-cols-2 w-full gap-x-2"}>
                {recoveryCodes.map((code, i) => (
                  <span
                    key={code}
                    className={i % 2 === 0 ? "text-right" : "text-left"}
                  >
                    {code}
                  </span>
                ))}
